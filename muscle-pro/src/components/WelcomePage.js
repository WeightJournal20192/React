...
const WelcomePage = ({
  ...
  const DeleteHandler = (exercise_name) => {
    axiosWithAuth()
      .delete(`api/user/delete`, exercise_name)
      .then(res => {
        setExercises(res.exercise_name);
      })
      ...
      {exercises.map(exercise => {
        return (
          <Card>
            ...
            <button onClick={() => DeleteHandler(exercises)}>delete</button>
          </Card>
        );
      })}
    </div>
